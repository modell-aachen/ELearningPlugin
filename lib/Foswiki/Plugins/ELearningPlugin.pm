...
sub _QWIKITOUR{
			...
			<script type="text/javascript">
				jQuery(document).ready(function() {				
					...
					$(\'#tour\').qwikitour({
						...
						autoPlay: false, 			// (true/ false) if true, plays the tour automaticaly 
	...
	# Load the Tour 
	my $tmpTopic = $topic =~ /AUTOINC/ ? 'WebHome' : $topic;
	my $tmpTopicName = $topic =~ /AUTOINC/ ? $web.'AUTOINC' : $topic;
	my( $meta2, $text2 ) = Foswiki::Func::readTopic( $web, 'WebHome' ); # needed to read the attachment of WebHome
	my @attachments = ($topic =~ /AUTOINC/) ? $meta2->find( 'FILEATTACHMENT' ) : $meta->find( 'FILEATTACHMENT' );
	
	foreach my $a ( @attachments ) {
		try {
			if($a->{name} =~  /($web)_($tmpTopicName)\.el/i){
				my $input = Foswiki::Func::readAttachment($web, $tmpTopic, $a->{name} );
				if(!($input =~ /<\s*script/si)){
					$output = $input.$tourpref;
				} 
			}
